...
import ParentLayoutShell from "./components/ParentLayoutShell";
...
export default async function ParentLayout({
  ...
  // 1. Enforce PARENT role strictly
  if (user.role !== "PARENT") {
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6">
        <div className="max-w-md w-full p-8 rounded-3xl bg-card border border-destructive/30 shadow-2xl text-center space-y-4">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            The Parent Portal is strictly restricted to accounts with the <strong className="text-foreground">PARENT</strong> role. Your current account role is <span className="font-mono font-bold text-primary">{user.role}</span>.
          </p>
          <div className="pt-2">
            <Link
              href="/dashboard"
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6">
        <div className="max-w-md w-full p-8 rounded-3xl bg-card border border-amber-500/30 shadow-2xl text-center space-y-4">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            This parent account is not linked to any active school tenant in the database. Please contact your institution administrator.
          </p>
          <div className="pt-2">
            <Link
              href="/dashboard"
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6">
        <div className="max-w-md w-full p-8 rounded-3xl bg-card border border-destructive/30 shadow-2xl text-center space-y-4">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            The school institution record associated with this parent account has been archived or deleted.
          </p>
        </div>
      </div>
    );
