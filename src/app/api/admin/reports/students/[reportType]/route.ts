...
export async function GET(
  ...
  try {
    let result;
    switch (reportType as StudentReportType) {
      case "student-list":
        result = await ReportServices.getStudentListReport(context, filters);
        break;
      case "class-list":
        result = await ReportServices.getClassListReport(context, filters);
        break;
      case "student-profile":
        result = await ReportServices.getStudentProfileReport(context, filters);
        break;
      case "admission":
        result = await ReportServices.getAdmissionReport(context, filters);
        break;
      case "transfer":
        result = await ReportServices.getTransferReport(context, filters);
        break;
      default:
        return NextResponse.json(
          { success: false, error: { message: `Unknown student report type: ${reportType}` } },
          { status: 404 }
        );
    }
    ...
    console.error(`Error generating student report (${reportType}):`, error);
    return NextResponse.json(
